...
module.exports = function () {
  ...
  function getPatientOncologySummary (request, callback) {
    ...
    getOncologyPatientReport(request, queryParts, callback);
  }
  
  function getPatientOncologyMedicationHistory (request, callback) {
    let queryParts = {
      columns: "DATE_FORMAT(t1.cur_onc_meds_start_date,'%d-%m-%Y') as meds_start_date, cur_onc_meds, cur_onc_meds_dose, cur_onc_meds_frequency",
      order  : [{
        column: 'encounter_datetime',
        asc   : false
      }]
  ...
  function getPatientOncologyDiagnosis (request, callback) {
    ...
    let queryParts = {
      columns: "DATE_FORMAT(t1.diagnosis_date,'%d-%m-%Y') as diagnosis_date, cancer_type, " +
        "(case " +
        "when cancer_stage=1 then 'Stage I' " +
        "when cancer_stage=2 then 'Stage II' " +
        "when cancer_stage=3 then 'Stage II' " +
        "when cancer_stage=4 then 'Stage IV' " +
        "end) as cancer_stage",
      order  : [{
  ...
  function getOncologyPatientReport(request, query, callback) {
    let patientUuid = request.params.uuid;
    let whereClause = ["uuid = ? ", patientUuid];
    ...
    let summaryDataPromise = db.queryDb(query);
    summaryDataPromise.then(callback).catch(callback);
  }
